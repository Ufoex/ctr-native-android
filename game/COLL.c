...
enum
{
	...
	COLL_SCRATCH_SEARCH_TRIANGLE_COUNT = 15,
};

struct CollScratchHostSlot
{
	struct ScratchpadStruct *owner;
	u64 lastUse;
	struct CollScratchHost host;
};
...
internal u32 CollScratch_CallbackPointerWord(CollThBuckCallback callback)
{
#if UINTPTR_MAX == UINT32_MAX
	u32 word = 0;
	memcpy(&word, &callback, sizeof(word));
	return word;
#else
	(void)callback;
	return 0;
...
{
	struct CollScratchHostSlot *emptySlot = NULL;
	struct CollScratchHostSlot *oldestSlot = &sCollScratchHostSlots[0];

	sCollScratchHostUseCounter++;

	for (int i = 0; i < COLL_SCRATCH_HOST_SLOT_COUNT; i++)
	{
		struct CollScratchHostSlot *slot = &sCollScratchHostSlots[i];
		...
		if (slot->lastUse < oldestSlot->lastUse)
		{
			oldestSlot = slot;
		}
	}
...
{
	sps->Union.ThBuckColl.threadPtr32 = CollScratch_ObjectPointerWord(thread);
	COLL_Scratch_GetHost(sps)->thread = thread;
}

void COLL_Scratch_SetCallback(struct ScratchpadStruct *sps, CollThBuckCallback callback)
{
	sps->Union.ThBuckColl.callbackPtr32 = CollScratch_CallbackPointerWord(callback);
	COLL_Scratch_GetHost(sps)->callback = callback;
}

void COLL_Scratch_SetMeshInfo(struct ScratchpadStruct *sps, struct mesh_info *meshInfo)
{
	sps->meshInfoPtr32 = CollScratch_ObjectPointerWord(meshInfo);
	COLL_Scratch_GetHost(sps)->meshInfo = meshInfo;
}

void COLL_Scratch_SetMeshInfo2(struct ScratchpadStruct *sps, struct mesh_info *meshInfo)
{
	sps->meshInfo2Ptr32 = CollScratch_ObjectPointerWord(meshInfo);
	COLL_Scratch_GetHost(sps)->meshInfo2 = meshInfo;
}

void COLL_Scratch_SetBspHitbox(struct ScratchpadStruct *sps, struct BSP *bsp)
{
	sps->bspHitboxPtr32 = CollScratch_ObjectPointerWord(bsp);
	COLL_Scratch_GetHost(sps)->bspHitbox = bsp;
}

void COLL_Scratch_SetCandidateQuadblock(struct ScratchpadStruct *sps, struct QuadBlock *quad)
{
	sps->candidate.quadblockPtr32 = CollScratch_ObjectPointerWord(quad);
	COLL_Scratch_GetHost(sps)->candidateQuadblock = quad;
}

void COLL_Scratch_SetHitQuadblock(struct ScratchpadStruct *sps, struct QuadBlock *quad)
{
	sps->hit.quadblockPtr32 = CollScratch_ObjectPointerWord(quad);
	COLL_Scratch_GetHost(sps)->hitQuadblock = quad;
}

void COLL_Scratch_SetBspHitboxHistory(struct ScratchpadStruct *sps, int index, struct BSP *bsp)
{
	if ((index < 0) || (index >= COLL_SCRATCH_HITBOX_HISTORY_COUNT))
	{
		return;
	}

	sps->bspHitboxesHitPtr32[index] = CollScratch_ObjectPointerWord(bsp);
	COLL_Scratch_GetHost(sps)->bspHitboxesHit[index] = bsp;
}

void COLL_Scratch_SetHitLevelVertex(struct ScratchpadStruct *sps, int index, struct LevVertex *vertex)
{
	if ((index < 0) || (index >= COLL_SCRATCH_HIT_TRIANGLE_VERTEX_COUNT))
	{
		return;
	}

	sps->hitLevelTriangle.vertexPtr32[index] = CollScratch_ObjectPointerWord(vertex);
	COLL_Scratch_GetHost(sps)->hitLevelTriangle[index] = vertex;
}
...
void COLL_Scratch_SetHitBspSearchVertex(struct ScratchpadStruct *sps, int index, struct BspSearchVertex *vertex)
{
	...
	sps->hitBspSearchTriangle.vertexPtr32[index] = CollScratch_ObjectPointerWord(vertex);
	COLL_Scratch_GetHost(sps)->hitBspSearchTriangle[index] = vertex;
}

void COLL_Scratch_SetSearchVertexLevel(struct ScratchpadStruct *sps, int index, struct LevVertex *vertex)
{
	if ((index < 0) || (index >= COLL_SCRATCH_SEARCH_VERTEX_COUNT))
	{
		return;
	}

	sps->bspSearchVert[index].levelVertexPtr32 = CollScratch_ObjectPointerWord(vertex);
	COLL_Scratch_GetHost(sps)->bspSearchVertLevelVertex[index] = vertex;
}
...
void COLL_Scratch_SetSearchTriangleQuadblock(struct ScratchpadStruct *sps, int index, struct QuadBlock *quad)
{
	...
	if ((index < 0) || (index >= COLL_SCRATCH_SEARCH_TRIANGLE_COUNT))
	{
		return;
	}

	ext->bspSearchTriangle[index].quadblockPtr32 = CollScratch_ObjectPointerWord(quad);
	COLL_Scratch_GetHost(sps)->bspSearchTriangleQuadblock[index] = quad;
}

internal void CollScratch_SelfTestCallback(struct ScratchpadStruct *sps, void *hitObject)
{
	(void)sps;
	(void)hitObject;
}
...
{
	struct ScratchpadStructExtended first = {0};
	struct ScratchpadStruct second = {0};
	struct Thread *thread = (struct Thread *)&first;
	struct mesh_info *meshInfo = (struct mesh_info *)&second;
	struct BSP *bsp = (struct BSP *)&first.cameraMatrix;
	struct QuadBlock *quad = (struct QuadBlock *)&first.cameraRot;
	struct LevVertex *levelVertex = (struct LevVertex *)&first.padding2C4;
	struct BspSearchVertex *searchVertex = &first.scratchpadStruct.bspSearchVert[8];
	struct CollScratchHost *firstHost;
	u32 expectedThreadWord;

	COLL_Scratch_SetThread(&first.scratchpadStruct, thread);
	COLL_Scratch_SetCallback(&first.scratchpadStruct, CollScratch_SelfTestCallback);
	COLL_Scratch_SetMeshInfo(&first.scratchpadStruct, meshInfo);
	COLL_Scratch_SetMeshInfo2(&first.scratchpadStruct, meshInfo);
	COLL_Scratch_SetBspHitbox(&first.scratchpadStruct, bsp);
	COLL_Scratch_SetCandidateQuadblock(&first.scratchpadStruct, quad);
	COLL_Scratch_SetHitQuadblock(&first.scratchpadStruct, quad);
	COLL_Scratch_SetBspHitboxHistory(&first.scratchpadStruct, 14, bsp);
	COLL_Scratch_SetHitLevelVertex(&first.scratchpadStruct, 2, levelVertex);
	COLL_Scratch_SetHitBspSearchVertex(&first.scratchpadStruct, 2, searchVertex);
	COLL_Scratch_SetSearchVertexLevel(&first.scratchpadStruct, 8, levelVertex);
	COLL_Scratch_SetSearchTriangleQuadblock(&first.scratchpadStruct, 14, quad);

#if UINTPTR_MAX == UINT32_MAX
	expectedThreadWord = (u32)(uintptr_t)thread;
#else
	expectedThreadWord = 0;
#endif

	firstHost = COLL_Scratch_GetHost(&first.scratchpadStruct);
	if ((sizeof(first.scratchpadStruct) != 0x20c) || (sizeof(first) != 0x2f8) ||
	    (first.scratchpadStruct.Union.ThBuckColl.threadPtr32 != expectedThreadWord) ||
	    (first.scratchpadStruct.Union.ThBuckColl.callbackPtr32 != CollScratch_CallbackPointerWord(CollScratch_SelfTestCallback)) ||
	    (first.scratchpadStruct.meshInfoPtr32 != CollScratch_ObjectPointerWord(meshInfo)) ||
	    (first.scratchpadStruct.bspHitboxPtr32 != CollScratch_ObjectPointerWord(bsp)) ||
	    (first.scratchpadStruct.hit.quadblockPtr32 != CollScratch_ObjectPointerWord(quad)) ||
	    (first.bspSearchTriangle[14].quadblockPtr32 != CollScratch_ObjectPointerWord(quad)) ||
	    (firstHost->thread != thread) || (firstHost->callback != CollScratch_SelfTestCallback) || (firstHost->meshInfo != meshInfo) ||
	    (firstHost->meshInfo2 != meshInfo) || (firstHost->bspHitbox != bsp) || (firstHost->candidateQuadblock != quad) ||
	    (firstHost->hitQuadblock != quad) || (firstHost->bspHitboxesHit[14] != bsp) ||
	    (firstHost->hitLevelTriangle[2] != levelVertex) || (firstHost->hitBspSearchTriangle[2] != searchVertex) ||
	    (firstHost->bspSearchVertLevelVertex[8] != levelVertex) || (firstHost->bspSearchTriangleQuadblock[14] != quad))
	{
		fprintf(stderr, "[CTR CollScratch] self-test failed: pointer synchronization\n");
		return 1;
	}

	COLL_Scratch_SetMeshInfo(&second, NULL);
	if ((COLL_Scratch_GetHost(&second)->meshInfo != NULL) || (COLL_Scratch_GetHost(&first.scratchpadStruct)->meshInfo != meshInfo))
	{
		fprintf(stderr, "[CTR CollScratch] self-test failed: owner isolation\n");
		return 1;
	}

#if UINTPTR_MAX == UINT32_MAX
	const char *pointerWordMode = "raw32";
#else
	const char *pointerWordMode = "zero64";
#endif
	printf("[CTR CollScratch] self-test passed: guest-size=0x20c extended=0x2f8 pointer-word=%s owners=isolated\n", pointerWordMode);
	return 0;
}
...
void COLL_SearchBSP_CallbackQUADBLK(const SVec3 *top, const SVec3 *bottom, struct ScratchpadStruct *sps, s32 hitRadius)
{
	...
	struct mesh_info *meshInfo = COLL_Scratch_GetHost(sps)->meshInfo;
	...
	COLL_SearchBSP_CallbackPARAM(MeshInfo_GetBspRoot(meshInfo, "COLL_SearchBSP quadblock root"), &sps->bbox,
...
u32 COLL_FIXED_INSTANC_TestPoint(struct ScratchpadStruct *sps, struct BSP *node)
{
	...
	{
		COLL_Scratch_SetBspHitbox(sps, node);
...
{
	struct BSP *bspArray = BSP_GetLeafHitboxes(node, "COLL_FIXED instance hitboxes");
	...
	for (/**/; bspArray->flag != 0; bspArray++)
	{
		...
		struct InstDef *instDef = BSP_GetInstDef(bspArray, "COLL_FIXED hitbox InstDef");
		...
		{
			if (bspArray == COLL_Scratch_GetHost(sps)->bspHitboxesHit[arraySize])
			{
				goto NextBSP;
			}
		}
		...
		if ((
		        ...
		        ((instance != NULL) && ((instance->flags & DRAW_COLLISION_MASK) != 0))) &&
...
void COLL_FIXED_BotsSearch(const SVec3 *posCurr, const SVec3 *posPrev, struct ScratchpadStruct *sps)
{
	...
	COLL_SearchBSP_CallbackPARAM(MeshInfo_GetBspRoot(COLL_Scratch_GetHost(sps)->meshInfo, "COLL_FIXED instance BSP"), &sps->bbox,
				     COLL_FIXED_BSPLEAF_TestInstance, sps);
}
                                                ...
                                                struct BspSearchVertex *v3, s32 normalZW)
{
	...
	struct QuadBlock *quad = COLL_Scratch_GetHost(sps)->candidateQuadblock;
	...
	COLL_Scratch_SetHitQuadblock(sps, quad);
	sps->hitBarycentrics.v1 = (s16)baryA;
	sps->hitBarycentrics.v2 = (s16)baryB;
	COLL_Scratch_SetHitLevelVertex(sps, 0, COLL_Scratch_GetHost(sps)->bspSearchVertLevelVertex[v1 - &sps->bspSearchVert[0]]);
	COLL_Scratch_SetHitLevelVertex(sps, 1, COLL_Scratch_GetHost(sps)->bspSearchVertLevelVertex[baryVertex1 - &sps->bspSearchVert[0]]);
	COLL_Scratch_SetHitLevelVertex(sps, 2, COLL_Scratch_GetHost(sps)->bspSearchVertLevelVertex[baryVertex2 - &sps->bspSearchVert[0]]);
...
internal void COLL_FIXED_QUADBLK_SetLoadScratchpadVertsContext(struct ScratchpadStruct *sps, struct QuadBlock *quad)
{
	...
	sCollFixedLoadScratchpadVertsVertexArray =
	    MeshInfo_GetVertices(COLL_Scratch_GetHost(sps)->meshInfo, "COLL_FIXED level vertices");
...
void COLL_FIXED_QUADBLK_LoadScratchpadVerts(struct ScratchpadStruct *sps)
{
	...
	for (const u16 *index = &ptrQuad->index[0]; index < &ptrQuad->index[9]; index++, bsv++)
	{
		...
		COLL_Scratch_SetSearchVertexLevel(sps, (int)(bsv - &sps->bspSearchVert[0]), vertCurr);
...
void COLL_FIXED_QUADBLK_TestTriangles(struct QuadBlock *quad, struct ScratchpadStruct *sps)
{
	...
	COLL_Scratch_SetCandidateQuadblock(sps, quad);
...
void COLL_FIXED_BSPLEAF_TestQuadblocks(struct BSP *node, struct ScratchpadStruct *sps)
{
	...
	struct QuadBlock *ptrQuad = BSP_GetLeafQuadBlocks(node, "COLL_FIXED leaf quad blocks");
	if (ptrQuad == NULL)
	{
		return;
	}
...
internal void COLL_FIXED_PlayerSearch_SetupSearch(struct ScratchpadStruct *sps, struct Driver *d)
{
	...
	COLL_Scratch_SetMeshInfo(sps, Level_GetMeshInfo(gGT->level1, "COLL_FIXED player mesh"));
...
{
	struct LevVertex *v0 = COLL_Scratch_GetHost(sps)->hitLevelTriangle[0];
	struct LevVertex *v1 = COLL_Scratch_GetHost(sps)->hitLevelTriangle[1];
	struct LevVertex *v2 = COLL_Scratch_GetHost(sps)->hitLevelTriangle[2];
...
internal b32 COLL_FIXED_PlayerSearch_CheckMaskGrabProgress(struct Driver *d, struct QuadBlock *quad)
{
	...
	struct CheckpointNode *restartPoints = Level_GetRestartPoints(level, "COLL checkpoint nodes");
	if ((restartPoints == NULL) || (quad->checkpointIndex >= level->cnt_restart_points))
	{
		return 0;
	}
	struct CheckpointNode *node = &restartPoints[quad->checkpointIndex];

	if (((d->actionsFlagSet & ACTION_BEHIND_START_LINE) == 0) &&
	    (node->nextIndex_forward > 1) &&
	    ((((restartPoints[0].distToFinish >> 2) << 3) <
	      (s32)(d->distanceToFinish_checkpoint - CTR_MipsMulLo(node->distToFinish, 8)))))
	{
		return 1;
	}

	u16 trackLength = restartPoints[0].distToFinish;
	...
	if ((node->distToFinish < (CTR_MipsMulLo(trackLength, 0xf) >> 4)) && (d->lastValid->checkpointIndex != 0xff) &&
	    ((restartPoints[d->lastValid->checkpointIndex].distToFinish + (trackLength >> 2)) < node->distToFinish))
	{
		return 1;
	}
...
void COLL_FIXED_PlayerSearch(struct Thread *t, struct Driver *d)
{
	...
	struct mesh_info *meshInfo = COLL_Scratch_GetHost(sps)->meshInfo;
	struct BSP *bspRoot = MeshInfo_GetBspRoot(meshInfo, "COLL_FIXED player BSP");
	if ((sps->boolDidTouchQuadblock == 0) && (bspRoot != NULL))
	{
	...
	if ((bspRoot != NULL) && (d->posCurr.y < CTR_MipsSll(CTR_MipsSubLo(bspRoot->box.min.y, 0x40), 8)))
	{
...
KeepNormal:;
	...
	COLL_Scratch_SetHitBspSearchVertex(sps, 0, v1);
	COLL_Scratch_SetHitBspSearchVertex(sps, 1, v2);
	COLL_Scratch_SetHitBspSearchVertex(sps, 2, v3);
	...
	COLL_Scratch_SetHitLevelVertex(sps, 0, COLL_Scratch_GetHost(sps)->bspSearchVertLevelVertex[v1 - &sps->bspSearchVert[0]]);
	COLL_Scratch_SetHitLevelVertex(sps, 1, COLL_Scratch_GetHost(sps)->bspSearchVertLevelVertex[v2 - &sps->bspSearchVert[0]]);
	COLL_Scratch_SetHitLevelVertex(sps, 2, COLL_Scratch_GetHost(sps)->bspSearchVertLevelVertex[v3 - &sps->bspSearchVert[0]]);
...
void COLL_MOVED_BSPLEAF_TestQuadblocks(struct BSP *node, struct ScratchpadStruct *sps)
{
	...
	s32 numQuads = node->data.leaf.numQuads;
	struct QuadBlock *ptrQuad = BSP_GetLeafQuadBlocks(node, "COLL_MOVED leaf quad blocks");
	if (ptrQuad == NULL)
	{
		return;
	}
...
void COLL_MOVED_FindScrub(struct QuadBlock *qb, s32 triangleID, struct ScratchpadStruct *sps)
{
	...
	for (s32 i = ext->numTriangles - 1; i >= 0; i--)
	{
		...
		if ((COLL_Scratch_GetHost(sps)->bspSearchTriangleQuadblock[i] == qb) && (tri->triangleID == triangleID))
		{
...

	{
		...
		COLL_Scratch_SetSearchTriangleQuadblock(sps, ext->numTriangles, qb);
...
{
	struct BSP *bsp = COLL_Scratch_GetHost(sps)->bspHitbox;
	struct Instance *linCInstance;
	s16 modelID;

	if ((bsp->flag & BSP_HITBOX_COLLIDABLE) != 0)
	{
		struct InstDef *instDef = BSP_GetInstDef(bsp, "COLL_MOVED hitbox InstDef");
		if (instDef == NULL)
		{
			return 1;
		}

		linCInstance = InstDef_GetInstance(instDef);
	...
	else
	{
		...
		struct InstDef *instDef = BSP_GetInstDef(bsp, "COLL_MOVED special hitbox InstDef");
		if (instDef == NULL)
		{
			return 1;
		}
		...
		struct Model *model = InstDef_GetModel(instDef, "COLL_MOVED special hitbox model");
		if (model == NULL)
		{
			return 1;
		}
...
{
	COLL_Scratch_SetBspHitboxHistory(sps, sps->numBspHitboxesHit, COLL_Scratch_GetHost(sps)->bspHitbox);
...
void COLL_MOVED_PlayerSearch(struct Thread *t, struct Driver *d)
{
	...
	struct mesh_info *meshInfo = Level_GetMeshInfo(gGT->level1, "COLL_MOVED player mesh");
	struct BSP *bspRoot = MeshInfo_GetBspRoot(meshInfo, "COLL_MOVED player BSP");
	...
	for (s32 iterations = 15; iterations != 0; iterations--)
	{
		...
		*blockedIterations = (sps->hitFraction > 0) ? 0 : *blockedIterations;
		...
		{
			// The sweep endpoints are posCurr >> 8, so a step under one world
			// unit lands on the same integer and looks like standing still. On
			// hardware that only happened when the kart really was stopped: a
			// 30fps step is four times longer, so anything moving cleared a
			// unit. Above that a slow kart produces sub-unit steps constantly,
			// and dropping them stops it dead rather than moving it slowly.
			//
			// There is nothing to sweep against at this size -- the step cannot
			// carry the kart through anything a full unit test would have caught
			// -- so apply it and let the next frame do the testing.
		...
		if (bspRoot != NULL)
		{
			COLL_SearchBSP_CallbackPARAM(bspRoot, &sps->bbox, COLL_MOVED_BSPLEAF_TestQuadblocks, sps);
		}
...
u32 COLL_MOVED_ScrubImpact(struct Driver *d, struct Thread *t, struct ScratchpadStruct *sps, struct Scrub *scrub, Vec3 *velocity)
{
	...
	struct QuadBlock *hitQuadblock = COLL_Scratch_GetHost(sps)->hitQuadblock;
	if ((d->vShiftCount != 0) && (sps->boolDidTouchQuadblock != 0) && ((hitQuadblock->quadFlags & QUADBLOCK_FLAG_GROUND) != 0) &&
	    (sps->hit.reorderResult != COLL_TRIANGLE_CLIP_FACE) && (hitQuadblock != d->underDriver))
	{
	...
	if (dot < 0)
	{
		...
		if ((scrubFlags & SCRUB_FLAG_APPLY_IMPACT) != 0)
		{
			...
			if ((sps->boolDidTouchQuadblock != 0) && ((sps->Union.QuadBlockColl.searchFlags & COLL_SEARCH_WALL_PROJECTION_DONE) == 0) &&
			    ((d->actionsFlagSetPrevFrame & ACTION_TOUCH_GROUND) == 0) && ((hitQuadblock->quadFlags & QUADBLOCK_FLAG_GROUND) != 0))
			{
				Vec3 wallVelocity;
